...
`/[locale]/admin` (route group `(admin)`) — client-side, `noindex`, no site
chrome. Supabase Auth email/password; write access is gated by the `admins`
...
- Editor toolbar writes the block syntax for the user (heading, bold, list,
  ...
  `admin/lib.ts` — buttons produce plain text, they don't switch the format.
- Images: paste, drag-drop, multi-file upload, a "gallery" mode that inserts
  ...
  (normal/wide/full), and an image library listing the `media` bucket for
  re-inserting an existing image at the caret.
