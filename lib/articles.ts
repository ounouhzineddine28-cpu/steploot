...
import {firstImageFromContent} from './article-content';
...
export async function getTrendingArticles(
  ...
  const {data, error} = await supabase
    ...
    .select('slug, category, title, excerpt, content, read_time, featured, published_at, cover_image_url')
    .eq('locale', locale)
  ...
  return ((data ?? []) as PublishedArticleRow[]).map((row) => ({
    ...
    // Fall back to the first image inside the article when no cover is set.
    coverImageUrl: row.cover_image_url || firstImageFromContent(row.content ?? '')
  }));
}
