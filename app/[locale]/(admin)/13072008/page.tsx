...
import {CardPreview, ContentArea, ImageField, btn, input, label} from './editor';
...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<'articles' | 'products' | 'pages' | 'content' | 'settings'>(
    'articles'
  );
...
function Articles({notify}: {notify: (m: string) => void}) {
  ...
  const load = useCallback(async () => {
    const {data} = await supabase
      .from('articles')
      .select('id, category, tag, sort_order, featured, status, published_at, cover_image_url, article_translations(locale, title)')
  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        sort_order: Number(article.sort_order) || 100,
      ...
      <div className="mb-6 grid gap-4 rounded-xl bg-bg-raised p-5 sm:grid-cols-3">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => setArticle({...article, sort_order: Number(e.target.value)})}
          />
          <p className="mt-1 text-[11px] text-text-faint">الأصغر يظهر أولًا (الافتراضي 100)</p>
        </div>
        ...
        <div className="sm:col-span-3">
          ...
          <p className="mt-5 mb-2 text-[11px] text-text-faint">هكذا تظهر البطاقة في الرئيسية:</p>
          <CardPreview
